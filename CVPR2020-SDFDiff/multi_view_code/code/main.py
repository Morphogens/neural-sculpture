...
prompt = "a bunny rabbit mesh rendered with maya zbrush"
clip_loss = ClipLoss(prompt)
...
# NOTE: read a file and create a sdf grid with target_grid_res
def read_txt(
    file_path,
    grid_res_x,
    grid_res_y,
...
def read_sdf(file_path, target_grid_res, target_bounding_box_min,
    ...
    with open(file_path) as file:
        ...
        loop = torch.cat((first_loop, second_loop, third_loop), 1).to(device)
...
def generate_image(bounding_box_min_x, bounding_box_min_y, bounding_box_min_z,
                   ...
                   height, grid, camera, back, camera_angle_list):
    ...
    outputs = renderer.ray_matching(w_h_3, w_h, grid, width, height,
                                    ...
                                    bounding_box_min_z, bounding_box_max_x,
                                    bounding_box_max_y, bounding_box_max_z,
                                    grid_res_x, grid_res_y, grid_res_z, eye_x,
                                    eye_y, eye_z)
...
if __name__ == "__main__":
    ...
    largest_distance_camera_grid = torch.sqrt(
        torch.pow(
            max(torch.abs(initial_camera_angle[0] - bounding_box_max_x),
                torch.abs(initial_camera_angle[0] - bounding_box_min_x)), 2) + torch.pow(
                    max(torch.abs(initial_camera_angle[1] - bounding_box_max_y),
                        torch.abs(initial_camera_angle[1] - bounding_box_min_y)), 2) + torch.pow(
    ...
    sdf_diff_list = []
    time_list = []
    image_loss = [1000] * num_cameras
    sdf_loss = [1000] * num_cameras
    iterations = 0
    scale = 1
    ...
    while (grid_res_x <= 64):
            ...
            for cam in range(num_cameras):
                ...
                while ((num < per_cam_iteration_count) and loss < prev_loss):
                    ...
                    image_initial = generate_image(
                        bounding_box_min_x, bounding_box_min_y,
                        bounding_box_min_z, bounding_box_max_x,
                        bounding_box_max_y, bounding_box_max_z, voxel_size,
                    ...
                    for cam in range(num_cameras):
                        ...
                        torchvision.utils.save_image(
                            image_initial,
                            "./" + out_dir + "final_cam_" + str(grid_res_x) +
                            "_" + str(cam) + "_" + str(i) + "-" + str(num) +
                            ".png",
                            nrow=8,
                            padding=2,
                            normalize=False,
        ...
        #             # img_path = "./" + out_dir + "final_cam_" + str(grid_res_x) + "_" + str(cam) + ".png"
        #             img_path = f"./{out_dir}/{grid_res_x}_{iter_i}_cam:{cam}.png"
        #             torchvision.utils.save_image(
        #                 image_initial, img_path, nrow=8, padding=2, normalize=False, range=None, scale_each=False, pad_value=0
        #             )
        # genetate result images
        # Save the final SDF result
        with open("./" + out_dir + str(grid_res_x) + "_best_sdf_bunny.pt",
                  'wb') as f:
