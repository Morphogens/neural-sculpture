...
def copy_optimizer_state(old_optimizer, new_optimizer, updated_grid_res):
    optim_state = old_optimizer.state_dict()['state']

    exp_avg = optim_state[0]['exp_avg'].clone()
    optim_state[0]['exp_avg'] = torch.nn.functional.interpolate(
        exp_avg[None, None, :],
        size=(updated_grid_res, ) * 3,
        mode='trilinear',
    )[0, 0, :]
    ...
    new_optimizer.state_dict()['state'] = optim_state
...
def grid_from_file(sdf_file_path, res):
    grid = np.load(sdf_file_path)
    # grid = np.stack([
...
class SDFOptimizer:
    # this connects the cursor pointer to the optimization,
    # weighting the gradient so only the desired locations are modified
    weight_grid: Optional[torch.Tensor]
    ...
    def parse_grid(
        ...
        self.grid_res_x, self.grid_res_y, self.grid_res_z = grid.shape
        self.voxel_size = Tensor([4. / (self.grid_res_x - 1)])
    ...
    def generate_initial_grid(self):
        if self.sdf_file_path is None:
            ...
            grid = grid_from_file(self.sdf_file_path, self.sdf_grid_res_list[0])
    ...
    def create_optimizer(self):
        ...
        # note(tal): we used to start with Adam. Was this important?
    ...
    def increase_res(self):
        ...
        if self.grid_res_idx + 1 >= len(self.sdf_grid_res_list):
            return

        update_grid_res = self.sdf_grid_res_list[self.grid_res_idx + 1]
        self.grid_res_x = self.grid_res_y = self.grid_res_z = update_grid_res
        self.voxel_size = Tensor([4. / (self.grid_res_x - 1)])
        ...
        new_optimizer = self.create_optimizer()
        copy_optimizer_state(old_optimizer, new_optimizer, update_grid_res)
    ...
    def set_coord(self,
        ...
        self.weight_grid = torch.zeros_like(self.grid)
        res = self.weight_grid.shape[0]
        self.weight_grid[
            max(0, x_coord -
    ...
    def optimization_step(self, prompt, coord=None):
        ...
        camera_angle_list = get_camera_angle_list(
        ...
        for cam_view_idx in range(0, num_cameras, step_size):
            init_loss = None
            tolerance = self.config.init_tolerance / (self.grid_res_idx + 1)


            cam_iter = 0
            for cam_iter in range(self.config.max_iters_per_cam):
                ...
                if self.global_iter % self.config.batch_size == 0:
                    ...
                    if self.weight_grid is not None:
                        coord_hook = self.grid.register_hook(
                            lambda grad: grad * self.weight_grid.float())
                    ...
                    print("")
                    print("sdf grid res:", self.sdf_grid_res, " - "
    ...
    def write_summaries(self, loss_dict, gen_img):
        ...
        for name, value in loss_dict.items():
            ...
            print(f"{name} loss: {value:.04f}")

        if self.results_dir:
            torchvision.utils.save_image(
                gen_img.detach(),
                "./" + self.results_dir + "/" + "final_cam_" +
                str(self.sdf_grid_res).zfill(4) + "-" +
